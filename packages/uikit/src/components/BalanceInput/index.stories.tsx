...
export const SiwtchUnits: React.FC = () => {
  const PEBBLE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"PEBBLE" | "USD">("PEBBLE");
  const conversionUnit = editingUnit === "PEBBLE" ? "USD" : "PEBBLE";
  const [values, setValues] = useState({
    PEBBLE: "1006.086957",
    USD: `${1006.086957 * PEBBLE_PRICE}`,
  });
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "PEBBLE" ? "USD" : "PEBBLE";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "PEBBLE") {
      setValues({
        PEBBLE: input,
        USD: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat * PEBBLE_PRICE}`,
      });
    } else {
      setValues({
        PEBBLE: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / PEBBLE_PRICE}`,
        USD: input,
      });
    }
  };
