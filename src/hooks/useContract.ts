...
export const useBalancerVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getBalancerVaultContract(library.getSigner()), [library])
}
...
export const usePredictionsContract = (address: string, tokenSymbol: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => {
    const getPredContract = tokenSymbol === 'PEBBLE' ? getCakePredictionsContract : getPredictionsContract
