...
import { GRAPH_API_PREDICTION_BNB, GRAPH_API_PREDICTION_PEBBLE } from 'config/constants/endpoints'
...
import { transformBetResponsePEBBLE, transformUserResponsePEBBLE } from './cakeTransformers'
...
import { BetResponsePEBBLE } from './cakeQueries'
...
export const transformBetResponse = (tokenSymbol) =>
  tokenSymbol === 'PEBBLE' ? transformBetResponsePEBBLE : transformBetResponseBNB

export const transformUserResponse = (tokenSymbol) =>
  tokenSymbol === 'PEBBLE' ? transformUserResponsePEBBLE : transformUserResponseBNB
...
export const getTotalWon = async (): Promise<{ totalWonBNB: number; totalWonPEBBLE: number }> => {
  const [{ market: BNBMarket, market: PEBBLEMarket }] = await Promise.all([
    request(
  ...
  const totalWonPEBBLE = getTotalWonMarket(PEBBLEMarket, 'PEBBLE')

  return { totalWonBNB, totalWonPEBBLE }
}
...
export const getBetHistory = async (
  ...
  api: string,
  tokenSymbol: string,
): Promise<Array<BetResponseBNB | BetResponsePEBBLE>> => {
