...
const Step1 = ({ hasProfile }: { hasProfile: boolean }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Box>
      <Text>
        {t(
          'The number of iPEBBLE equals the locked staking amount if the staking duration is longer than %weeks% weeks. If the staking duration is less than %weeks% weeks, it will linearly decrease based on the staking duration.',
          {
            weeks: weeksDisplay,
          },
        )}
  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Lock PEBBLE in the PEBBLE pool')}
      </Heading>
      <Box>
        <Text mb="4px" color="textSubtle" small>
          {t(
            'The maximum amount of PEBBLE you can commit to the Public Sale equals the number of your iPEBBLE. Lock more PEBBLE for longer durations to increase the maximum PEBBLE you can commit to the sale.',
          )}
        </Text>
        <TooltipText as="span" fontWeight={700} ref={targetRef} color="textSubtle" small>
          {t('How does the number of iPEBBLE calculated?')}
        </TooltipText>
        <Text mt="4px" color="textSubtle" small>
          {t(
            'Missed this IFO? You will enjoy the same amount of iPEBBLE for future IFOs if your locked-staking position is not unlocked.',
          )}
        </Text>
      </Box>
      {hasProfile && (
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t('When the IFO sales are live, you can “commit” your PEBBLE to buy the tokens being sold.')} <br />
      </Text>
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent PEBBLE tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )
