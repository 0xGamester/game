...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of PEBBLE up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max PEBBLE entry is capped by minimum between your average PEBBLE balance in the iPEBBLE, or the pool’s hard cap. To increase the max entry, Stake more PEBBLE into the iPEBBLE',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max PEBBLE entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iPEBBLE.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock more PEBBLE for longer durations to increase the maximum number of PEBBLE you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max PEBBLE entry is capped by your average PEBBLE balance in the iCake. To increase the max entry, Stake more PEBBLE into the iCake',
      )
    )
  ...
  const label = isCurrencyCake ? t('Max. PEBBLE entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('PEBBLE to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
