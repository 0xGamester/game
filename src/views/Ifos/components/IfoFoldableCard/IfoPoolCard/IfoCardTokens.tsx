...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough PEBBLE to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your PEBBLE.',
    ),
    { placement: 'bottom' },
  )
    ...
    if (
      ...
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">{t('You don’t have any iPEBBLE available for IFO public sale.')}</MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, lock some PEBBLE in the fixed-term staking PEBBLE pool!')}
              </Text>
