...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.PEBBLE].includes(initToken)) {
      return initToken
    }

    return PredictionSupportedSymbol.PEBBLE
  })

  useEffect(() => {
    const upperToken = _toUpper(token as string) as PredictionSupportedSymbol

    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.PEBBLE].includes(upperToken)) {
