...
import { ADMINS, PANPEBBLE_SPACE, SNAPSHOT_VERSION } from './config'
...
const STRATEGIES = [{ name: 'cake', params: { symbol: 'PEBBLE', address: tokens.cake.address, decimals: 18, max: 300 } }]
const NETWORK = '56'
...
export const getVotingPower = async (account: string, poolAddresses: string[], blockNumber?: number) => {
  ...
  const [total] = await getScores(PANPEBBLE_SPACE, STRATEGIES, NETWORK, [account], blockNumber)
...
export async function getVotingPowerByCakeStrategy(voters: string[], blockNumber: number) {
  const strategyResponse = await getScores(PANPEBBLE_SPACE, STRATEGIES, NETWORK, voters, blockNumber)
