...
const PredictionCardContent = () => {
  ...
  const cakeWonInUsd = multiplyPriceByAmount(cakePriceBusd, data?.totalWonPEBBLE || 0)

  const localisedBnbUsdString = formatLocalisedCompactNumber(bnbWonInUsd + cakeWonInUsd)
  const bnbWonText = t('$%wonInUsd% in BNB + PEBBLE won so far', { wonInUsd: localisedBnbUsdString })
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="#280D5F" mb="40px">
          {t('Predict the price trend of BNB or PEBBLE to win')}
        </Text>
      </Flex>
