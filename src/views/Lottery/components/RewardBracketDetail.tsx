...
const RewardBracketDetail: React.FC<RewardBracketDetailProps> = ({
  ...
  const [cakePriceBusd, setCakePriceBusd] = useState(0)
  const bnbPriceBusd = usePriceBnbBusd()

  useEffect(() => {
    const initialize = async () => {
      const cakePriceBusd = await usePriceCakeTokenBusd()
      setCakePriceBusd(Number(cakePriceBusd) * bnbPriceBusd.toNumber())
    }
    ...
    <Flex flexDirection="column">
      ...
      <>
        {isLoading || cakeAmount.isNaN() ? (
          ...
          <Balance fontSize="20px" bold unit=" PEBBLE" value={getBalanceNumber(cakeAmount)} decimals={0} />
        )}
        {isLoading || cakeAmount.isNaN() ? (
          <>
          ...
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(cakeAmount.div(parseInt(numberWinners, 10)), 18, 2)} PEBBLE {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
