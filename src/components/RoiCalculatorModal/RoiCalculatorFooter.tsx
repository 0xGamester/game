...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of PEBBLE rewards each farm receives, as a proportion of the PEBBLE produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 PEBBLE per block, a 40x farm would receive 40 PEBBLE per block.')}
      </Text>
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            {!Number.isFinite(apy) ? (
              <Text color="textSubtle" small>
                {isFarm ? t('Base APR (PEBBLE yield only)') : t('APR')}
              </Text>
            ) : (
              <Text color="textSubtle" small>
                {t('APY')}
